...
describe('Platform Android', function () {
    it('should be android', function () {
        expect(helper.isAndroid).to.equal(true);
        expect(helper.isIos).to.equal(false);
    });
    describe('either', function () {
        ...
        it('can handle numbers', function () {
            expect(helper.either(1, 2)).to.equal(2);
        });
        it('can handle one number', function () {
            expect(helper.either(2)).to.equal(null);
        });
        it('can handle strings', function () {
            expect(helper.either('hello', 'world')).to.equal('world');
        });
        it('can handle arrays', function () {
            helper.either([1, 2, 3], [4, 5, 6]).should.be.deep.equal([4, 5, 6]);
        });
    });
    describe('invoke either', function () {
        ...
        it('can handle two functions which return a value', function () {
            ...
            expect(helper.invokeEither(first, second)).to.equal(200);
        });
        it('can handle two closures which return a', function () {
            ...
            expect(helper.invokeEither(area(10), cube(5))).to.equal(125);
        });
        it('can handle nulls', function () {
